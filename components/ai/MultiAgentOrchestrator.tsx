...
export default function MultiAgentOrchestrator() {
  const { agents, tasks, messages, summary, loading, orchestrateWorkflow } = useAgentActivity(3000)
  ...
  const handleOrchestrate = async (workflowType: string) => {
    ...
    try {
      await orchestrateWorkflow(workflowType)
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="space-y-3">
          {tasks.slice(-10).reverse().map((task, index) => (
            <motion.div
              key={task.id}
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
